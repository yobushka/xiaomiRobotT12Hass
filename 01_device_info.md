...
1. **Уборка по комнатам:** Используется `siid: 7, aiid: 3`. Команды принимаются стабильно.
2. **ID комнат:** Найдены в логах (`log.json`) и подтверждены тестами: 10, 11, 12, 13.
3. **Управление параметрами:** Используется `xiaomi_miot.set_miot_property`.

## Особенности (Known Issues / Notes)

1. **xiaomi_cloud_map_extractor:** По-прежнему сложности с авторизацией, карта в HA может не отображаться.
2. **Обновление настроек:** Пылесос может игнорировать изменение мощности/режима, если находится в режиме `idle` на зарядке (особенность прошивки).
3. **Команда Стоп:** Для гарантированной остановки использовать `siid: 2, aiid: 2`.
